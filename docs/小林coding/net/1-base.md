...
>   | 其他网络接口 | 如蓝牙网络等           |
>
>   ![image-20250521120915905](./1-base.assets/image-20250521120915905.png)
...
在发送数据包时，如果目**标主机不是本地局域网，填入的 Mac 地址是路由器**，也就是把数据包转发给路由器，路由器一直转发下一个路由器，直到转发到目标主机的路由器，发现 IP 地址是自己局域网内的主机，就会 ARP 请求获取目标主机的 Mac 地址，从而转发到这个服务器主机。
...
>
> - 应用调用 `send()`。
> - 内核分配 `sk_buff`，将用户数据拷贝进去。
>
> ### 2. TCP 层处理
>
> - `sk_buff` 放入发送缓冲区。
>
> - **TCP 协议复制一个 `sk_buff` 副本**用于发送，原始留作重传。
>
> - ### （第二次拷贝）
>
> - 填写 TCP 头部。
...
> - sk_buff 加入驱动发送队列。
>
> ### 5. 触发软中断 → DMA 映射 → 放入 RingBuffer
>
> - 软中断触发 `ksoftirqd` 线程发送。
> - DMA 映射 skb->data。
> - 数据挂入网卡的发送 RingBuffer。
>
> ### 6. 硬中断：发送完成回收资源
>
> - 网卡触发硬中断。
> - 释放 DMA 映射和 sk_buff 资源。
>
> ### 7. 接收 ACK → TCP 释放原始 sk_buff
>
> - TCP 层收到 ACK。
> - 删除原始保留的 sk_buff，实现可靠性。
>
> ### 涉及关键机制
>
> | 组件       | 作用说明                                           |
> | ---------- | -------------------------------------------------- |
> | `sk_buff`  | 核心数据结构，跨层复用，避免频繁拷贝。             |
> | DMA        | 避免 CPU 介入数据搬运，直接由设备访问内存。        |
> | 软中断     | 推迟耗时操作，提升效率（`ksoftirqd` 线程处理）。   |
> | 硬中断     | 网卡通知发送完成，释放资源。                       |
> | RingBuffer | 网卡驱动和硬件共享的环形缓冲区，用于发送数据管理。 |
>
> ### 总结：三次内存拷贝位置
>
> 1. **用户态 → 内核态（分配 sk_buff）**
> 2. **TCP 层创建副本 sk_buff 用于发送**
> 3. **IP 层大于 MTU 分片（拆分为多个 sk_buff）**
>
> - > #### 总结：为什么不能只用一个 sk_buff + 指针进行分片？
>   >
>   > | 原因                       | 解释                                        |
>   > | -------------------------- | ------------------------------------------- |
>   > | 每个分片是独立 IP 包       | 需要独立 sk_buff 来封装头部、控制结构       |
>   > | sk_buff 是完整网络包的载体 | 包括路由、队列、缓冲、控制信息              |
>   > | 原始 sk_buff 不能动        | 要保留用于 TCP 重传                         |
>   > | 并发处理风险高             | 指针表示多个逻辑包会引发多线程/中断处理冲突 |
>   > | 代码可维护性差             | 分片逻辑变得复杂且容易出错                  |
>   >
>   > #### 总结：为什么不能只重传某个分片?
>   >
>   > | 问题                                      | 回答                                                         |
>   > | ----------------------------------------- | ------------------------------------------------------------ |
>   > | TCP 重传是按 segment 重传还是按 IP 分片？ | 按 segment 重传                                              |
>   > | IP 分片丢了，能只补发那个分片吗？         | 不能，只能重发整个 segment                                   |
>   > | 为什么不能？                              | 因为 IP 不可靠、TCP 不知道分片情况，接收方也不反馈哪个分片丢了 |
>   > | 重传时还会重新分片吗？                    | 会，只要 segment > MTU，IP 层还是会分片                      |
>
> ### 在网络模型里, 一共切分切分几次?
>
> **只有 IP 层才会“分片”**，**TCP 层不会分片，而是“拆段（segment）”**。
>  所以：
>
>  **TCP 拆段**（segment） 是一次
>  **IP 分片**（fragmentation） 可能会有一次（视情况而定）
>   总共：**“最多”发生两次“切分”动作，但“分片”这个术语只属于 IP 层**
>
> ### 重传是那层?
>
> 重传只发生在 **tcp层**, 对于ip层
>
> 那就是 ip分片少一个, 会重传 这个tcp段, 也就是又要重新分片传输
