...
> ## hzh-c
>
> ### 已建立的 TCP 连接中，客户端宕机后恢复的处理总结
>
> #### 1. **客户端的 SYN 报文中的源端口与历史连接不同**
> - **服务端处理**：
>   - 服务端认为是新的连接请求，正常进行三次握手，建立新的连接。
> - **旧连接的处理**：
>   - 如果服务端发送数据包给客户端：
>     - 客户端因连接已关闭，返回 RST 报文。
>     - 服务端收到 RST 报文后释放旧连接。
>   - 如果服务端没有发送数据包：
>     - TCP 保活机制启动，检测到客户端无响应后，释放旧连接。
>
> #### 2. **客户端的 SYN 报文中的源端口与历史连接相同**
> - **服务端处理**：
>   - 服务端处于 `Established` 状态，收到 SYN 报文（乱序的 SYN 报文）。
>   - 服务端回复一个携带正确序列号和确认号的 **Challenge ACK** 报文。
> - **客户端处理**：
>   - 客户端收到 Challenge ACK，发现确认号（`ack num`）与期望值不符。
>   - 客户端返回 RST 报文。
>   - 服务端收到 RST 报文后，释放旧连接。
>
> #### 3. **总结**
