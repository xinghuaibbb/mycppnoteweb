...
> ### 总结
>
> - **TCP 和 UDP 可以同时绑定相同的端口号**，因为它们是独立的传输层协议模块，端口号互不冲突。
> - 数据包通过 IP 包头的「协议号」字段区分后，分别交由 TCP 或 UDP 模块处理。
> - TCP 的端口和 UDP 的端口是独立的，例如 TCP 的 80 端口和 UDP 的 80 端口可以同时存在，互不干扰。
...
> ## hzh-c
>
> ### 多个 TCP 服务进程绑定同一个端口的总结
>
> #### 1. **默认情况下的行为**
> - **同一个 IP 地址和端口**：
>   - 如果两个 TCP 服务进程绑定相同的 IP 地址和端口，执行 `bind()` 会报错：`Address already in use`。
> - **不同的 IP 地址但相同的端口**：
>   - 如果两个 TCP 服务进程绑定不同的 IP 地址但端口相同，可以绑定成功。
>   - 但如果一个进程绑定了 `0.0.0.0`（代表所有 IP 地址），另一个进程绑定特定 IP 地址和相同端口，则会冲突，`bind()` 报错。
>
> ---
>
> #### 2. **解决方法**
...
>   - 适用于负载均衡场景，但需要内核版本 >= 3.9。
> - **SO_REUSEADDR**：
>   - 允许绑定处于 `TIME_WAIT` 状态的 IP+PORT。
>   - 允许绑定逻辑上包含的 IP 地址（如 `0.0.0.0:8888` 和 `192.168.1.100:8888`）。
>
> ---
>
> #### 3. **TIME_WAIT 状态导致的绑定失败**
> - **原因**：
>   - 服务端重启时，旧连接可能处于 `TIME_WAIT` 状态，导致 IP+PORT 被占用。
> - **解决方法**：
>   - 在 `bind()` 前设置 `SO_REUSEADDR` 属性，允许绑定处于 `TIME_WAIT` 状态的 IP+PORT。
>
> ---
>
> #### 4. **最佳实践**
> - 在所有 TCP 服务端程序中，调用 `bind()` 前设置 `SO_REUSEADDR` 属性：
>   ```c
>   int on = 1;
>   setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on));
>   ```
> - 如果需要多个进程绑定相同的 IP 和端口，使用 `SO_REUSEPORT` 属性（需内核支持）。
>
> ---
>
> #### 5. **总结**
> - 默认情况下，多个 TCP 服务进程不能绑定相同的 IP 地址和端口。
> - 使用 `SO_REUSEADDR` 或 `SO_REUSEPORT` 可以解决绑定冲突问题，具体选择取决于场景需求。
> - 设置 `SO_REUSEADDR` 是 TCP 服务端程序的推荐实践，能有效避免重启时的绑定失败问题。
...
>
> #### 1. **默认行为**
> - **同一 IP 和端口**：多个 TCP 服务进程不能绑定相同的 IP 地址和端口，`bind()` 会报错：`Address already in use`。
> - **不同 IP 同一端口**：可以绑定成功，但如果一个进程绑定 `0.0.0.0`（代表所有 IP 地址），另一个绑定特定 IP 地址和相同端口，则会冲突。
>
> ---
>
> #### 2. **解决方法**
...
>   - 允许绑定逻辑上包含的 IP 地址（如 `0.0.0.0:8888` 和 `192.168.1.100:8888`）。
> - **SO_REUSEPORT**：
>   - 允许多个进程绑定相同的 IP 和端口（需内核版本 >= 3.9）。
>   - 常用于负载均衡场景。
>
> ---
>
> #### 3. **TIME_WAIT 状态问题**
> - **原因**：服务端重启时，旧连接可能处于 `TIME_WAIT` 状态，导致 IP+PORT 被占用。
> - **解决**：设置 `SO_REUSEADDR` 属性，允许绑定处于 `TIME_WAIT` 状态的 IP+PORT。
>
> ---
>
> #### 4. **最佳实践**
> - 在 TCP 服务端程序中，调用 `bind()` 前设置 `SO_REUSEADDR` 属性：
>   ```c
>   int on = 1;
>   setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on));
>   ```
> - 如果需要多个进程绑定相同的 IP 和端口，使用 `SO_REUSEPORT`。
>
> ---
>
> #### 5. **总结**
> - 默认情况下，多个 TCP 服务进程不能绑定相同的 IP 和端口。
> - 使用 `SO_REUSEADDR` 或 `SO_REUSEPORT` 可以解决绑定冲突问题。
> - 设置 `SO_REUSEADDR` 是推荐实践，可避免重启时的绑定失败问题。
...
>
> 1. **客户端端口是否可以重复使用**  
>    - **可以**，因为 TCP 连接是通过四元组（源 IP、源端口、目的 IP、目的端口）唯一标识的。只要四元组中任意一个元素不同，就可以重复使用同一个端口。
>
> 2. **多个客户端是否可以绑定同一个端口**  
>    - **不可以**，如果多个客户端绑定的 IP 和端口完全相同，`bind()` 会报错 "Address already in use"。
>    - 如果绑定的 IP 不同（如绑定到不同的网卡），则可以绑定相同的端口。
>
> 3. **TIME_WAIT 状态过多导致端口耗尽问题**  
>    - 如果客户端与同一个服务器建立大量连接，TIME_WAIT 状态过多可能导致端口耗尽，无法建立新连接。
>    - 如果连接的服务器不同，端口资源可以重复使用，不会有问题。
>
> 4. **解决 TIME_WAIT 导致的端口耗尽问题**  
>    - 开启内核参数 `net.ipv4.tcp_tw_reuse`：
>      - 允许重用处于 TIME_WAIT 状态且持续时间超过 1 秒的连接。
>      - 仅对客户端（连接发起方）有效，服务端无效。
>
> 5. **客户端端口选择流程**  
>    - 默认情况下，客户端调用 `connect()` 时，内核会从 `net.ipv4.ip_local_port_range` 范围内随机选择一个端口。
>    - 如果需要指定端口，可以通过 `bind()` 函数绑定端口后再调用 `connect()`。
>    - 开启 `tcp_tw_reuse` 后，TIME_WAIT 状态的端口在特定条件下可以被重用。
>
> ### 注意事项
> - 客户端端口资源有限，但通过合理配置（如开启 `tcp_tw_reuse`）和优化，可以避免端口耗尽问题。
> - 通常不建议客户端手动绑定端口，交由内核自动选择更为高效和安全。
