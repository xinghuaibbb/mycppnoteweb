...
> ### TCP 重传机制总结
>
> TCP 为了实现可靠传输，设计了多种重传机制来应对数据丢失或网络延迟问题。以下是主要的重传机制及其特点：
>
> #### 1. **超时重传**
> - **原理**：发送方设置定时器，若在超时时间内未收到 ACK，则重传数据。
> - **关键点**：
>   
>   - 超时时间 (Retransmission Timeout `RTO`) 应动态调整，略大于 RTT（Round-Trip Time 往返时延）。
>   
>     - `RTT` 指的是**数据发送时刻到接收到确认的时刻的差值**，也就是包的往返时间
>   - RTO 的计算公式基于平滑 RTT (`SRTT`) 和 RTT 波动范围 (`DevRTT`)。
>   - 若多次超时，RTO 会**指数倍增长**以减少网络拥塞。
>   
>     - > 若多次超时，TCP 的超时时间 (`RTO`) 会**指数倍增长**。这是 TCP 的一种拥塞控制策略，称为**指数退避**（Exponential Backoff）。
>       >
>       > ##### 原理
>       >
>       > - 每次超时重传后，TCP 会将当前的超时时间 `RTO` 加倍。
>       > - 这种机制的目的是在网络状况较差时，减少频繁重传对网络的进一步拥塞。
>       >
>       > ##### 目的
>       >
>       > - 避免在网络拥塞时频繁重传，进一步加剧拥塞。
>       > - 给网络更多时间恢复正常。
>       >
>       > ##### 限制
>       >
>       > - 为了避免 `RTO` 无限增长，TCP 通常会设置一个最大超时时间（`RTO_MAX`）。
>
> #### 2. **快速重传**
> - **原理**：发送方收到 **3 个重复的 ACK** 时，**立即重传**丢失的数据包，无需等待超时。
>   - 由于 缺失了 中间某个包, ACK一直是 那个, 而不是别的
>
> - **优点**：比超时重传更快地发现和解决数据丢失问题。
> - **问题**：**无法精确判断哪些数据包丢失**，可能导致效率低下或资源浪费。
>   - 重传范围不明确
>     - 如果**只重传一个丢失的**数据包（如 Seq2），可能导致**后续丢失的数据包**（如 Seq3）**需要再次触发**重传，效率低下。
>     - 如果**重传所有未确认**的数据包（如 Seq2 ~ Seq6），可能会**重复发送已被接收方接收**的数据（如 Seq4 ~ Seq6），浪费网络资源。
>
>
> #### 3. **SACK（选择性确认）**
> - **原理**：接收方通过 **TCP 头部的 SACK 选项**告知发送方已接收的具体数据段。
> - **优点**：发送方只需重传丢失的数据段，避免重复传输已接收的数据。
> - **要求**：发送方和接收方**都需支持** SACK。
> -  `net.ipv4.tcp_sack` 参数
>
> #### 4. **D-SACK（重复选择性确认）**
> - `D-SACK`（Duplicate SACK）是 TCP 的一种扩展机制，用于**通过选择性确认（SACK）**告知发送方哪些数据包被**重复接收**。它的主要作用是帮助发送方分析网络问题并优化传输。
> - **作用:** 
>   - **诊断丢包原因**：区分是数据包丢失还是 ACK 丢失。
>   - **检测网络延迟**：识别因网络延迟导致的重复数据包。
>   - **检测数据包复制**：发现网络中是否存在数据包的重复传输。
>
> - **优势**
>   - 提供更详细的网络状态反馈。
>   - 帮助发送方优化重传策略，减少不必要的重传。
> - **配置**
>   - 在 Linux 系统中，`D-SACK` 默认启用，可通过 `net.ipv4.tcp_dsack` 参数控制。
>
>
> #### `D-SACK` 是 TCP 协议中提升可靠性和效率的重要机制，尤其在复杂网络环境中表现出色。
>
> 
>
> #### 总结
> - **超时重传**适用于一般丢包场景，但可能延迟较高。
> - **快速重传**通过重复 ACK 提高了重传效率。
> - **SACK** 和 **D-SACK** 提供了更精确的重传和网络状态反馈，进一步优化了 TCP 的可靠性和效率。
>
> 这些机制共同构成了 TCP 在复杂网络环境中实现可靠传输的核心能力。
...
> ### 滑动窗口总结
>
> 滑动窗口是 TCP 协议中用于提高传输效率的重要机制，通过允许发送方在未收到确认（ACK）之前连续发送多个数据包，减少了等待时间，提高了网络利用率。
>
> #### 核心概念
> 1. **窗口大小**：  
>    - 表示发送方**无需等待确认即可连续发送的最大数据量**。
>    - 由接收方通过 TCP 报文中的 `Window` 字段告知发送方。
>
> 2. #### **累计确认**：  !!!!
>
>    - 接收方通过 ACK 确认所有小于当前序列号的数据已接收，即使中间有部分 ACK 丢失，也能通过后续 ACK 确认。
>
> 3. **组合重传效果**
>
>    - **滑动窗口** 提高了数据传输的并发性，减少了等待时间。
>    - **SACK** 减少了重传的冗余数据量，提升了重传效率。
>    - **D-SACK** 提供了网络状态的反馈信息，帮助发送方进一步优化传输策略。
>
> #### 发送方的滑动窗口
> - **窗口划分**：
>   1. 已发送并收到 ACK 的数据。
...
>   - `SND.NXT`：指向未发送但可发送范围的第一个字节。
>   - `SND.WND`：发送窗口大小，由接收方决定。
>   - 可用窗口大小是计算出来的: `SND.WND`-(`SND.NXT`-`SND.UNA`)
>
> #### 接收方的滑动窗口
> - **窗口划分**：
>   1. 已接收并确认的数据。
...
>   3. 未接收且不可接收的数据。
> - **关键指针**：
>   - `RCV.NXT`：指向期望接收的下一个字节。
>   - `RCV.WND`：接收窗口大小，通告给发送方。
>
> #### 特点
> 1. **动态调整**：  
>    - 窗口大小会根据接收方的处理能力动态变化，发送方通过接收方的通告调整发送速率。
> 2. **发送窗口与接收窗口的关系**：  
>    - 发送窗口大小约等于接收窗口大小，但存在一定的时延差异。
>
> #### 优势
> - 提高了数据传输效率，减少了等待时间。
> - 通过累计确认和动态窗口调整，适应不同网络状况和接收方处理能力。
>
> 滑动窗口机制是 TCP 实现可靠、高效传输的核心之一。
...
>
> 1. **缓冲区影响窗口大小**：  
>    - **接收方的缓冲区未及时读取数据**时，接收窗口会逐渐缩小，最终可能关闭（窗口大小为 0）。
>    - **操作系统直接减少接收缓冲区大小**时
>      - 接收方 通知调整 窗口大小
>      - 若 发送方 窗口未及时调整，发过来的大于 接收方此时要求的, 可能导致数据包丢失。
>      - 然后 发送方 收到调整, 但此时本身 窗口就小, 再减少, 可能会**负值** 
>    
> 2. **窗口探测机制**：  
>    - 当接收窗口为 0 时，发送方会定期发送窗口探测报文，检查接收窗口是否重新打开。
>
> 3. **避免丢包的策略**：  
>    - TCP 规定：**先收缩窗口，再减少缓冲区大小**，以避免因窗口调整不及时而导致的数据丢失。
>
> ### 核心意义
> 操作系统缓冲区的状态直接影响滑动窗口的动态调整，TCP 通过窗口探测和调整策略，确保数据传输的可靠性和稳定性。
>
> 
>
> ### 窗口关闭总结
>
> 1. **窗口关闭的定义**：  
>    - 当接收方通告的窗口大小为 0 时，发送方会停止发送数据，直到接收方通告一个非 0 窗口大小为止。
>
> 2. **潜在危险**：  
>    - 如果接收方通告非 0 窗口的 ACK 报文在网络中丢失，发送方会一直等待窗口打开的通知，而接收方会一直等待数据，可能导致**死锁**。
>
> 3. **TCP 的解决方案**：  
>    - **持续计时器**：  
>      - 当发送方收到零窗口通知时，启动持续计时器。
>      - 如果计时器超时，发送方会发送**窗口探测报文**（Window Probe）。
>    - **窗口探测报文**：  
>      - 探测接收方的窗口状态。
>      - 如果窗口仍为 0，重新启动计时器；如果窗口非 0，发送方恢复数据传输。
>
> 4. **窗口探测机制**：  
>    - 通常探测次数为 3 次，每次间隔 30-60 秒（具体实现可能不同）。
>    - 如果多次探测后窗口仍为 0，有些 TCP 实现会发送 `RST` 报文中断连接。
>
> ### 核心意义
> 通过持续计时器和窗口探测机制，TCP 有效避免了窗口关闭时的死锁问题，确保连接的可靠性和稳定性。
>
> 
>
> ### 糊涂窗口综合症总结
>
> **糊涂窗口综合症**是由于发送方和接收方**频繁传输小数据包**，**导致网络利用率低下**的问题。
>
> #### **问题来源**
> 1. **接收方**：通告一个很小的窗口，导致发送方只能发送少量数据。
> 2. **发送方**：发送小数据包，浪费了大量传输开销。
>
> #### **解决方案**
> 1. **接收方策略**：  
>    
>    - 当窗口大小小于 `min(MSS, 缓存空间/2)` 时，通告窗口为 `0`，阻止发送方发送数据。
>
>      > - **MSS** 是指 **TCP 每次发送的最大数据段大小**，它表示 TCP 数据部分的最大字节数，不包括 TCP 和 IP 头部。
>      > - 假设 MTU 为 1500 字节：
>      >   - IP 头部：20 字节
>      >   - TCP 头部：20 字节
>      >   - MSS = 1500 - 20 - 20 = **1460 字节**
>    - 等窗口大小 >= `MSS` 或缓存空间有一半可用时，再通告非 0 窗口。
>    
> 2. **发送方策略**：  
>    - 使用 **Nagle 算法**，延迟发送小数据，直到：
>      - 可用窗口大小 >= `MSS` 且数据大小 >= `MSS`。
>      - 或收到之前发送数据的 ACK。
>
> #### **注意**
> - **接收方和发送方需配合**，才能有效避免糊涂窗口综合症。
> - 对于需要快速交互的小数据包（如 `telnet` 或 `ssh`），可以关闭 Nagle 算法。
>
> #### **总结**
> 通过**接收方不通告小窗口**和**发送方使用 Nagle 算法**，TCP 可以避免频繁传输小数据包，提高网络利用率。
...
> ### 拥塞控制总结
>
> 1. **目的**：避免发送方的数据填满网络，防止网络拥塞导致的恶性循环（时延增加、丢包、重传等问题）。
>
> 2. **核心机制**：
>    - 引入 **拥塞窗口 (cwnd)**，动态调整发送数据量。
>    - 发送窗口 `swnd = min(cwnd, rwnd)`，受拥塞窗口和接收窗口共同限制。
>
> 3. **判断拥塞**：发送方未在规定时间内收到 ACK（超时重传）即认为网络拥塞。
>
> 4. **四种算法**：
>    - **慢启动**：`cwnd` 指数增长，直到达到慢启动阈值。
>    - **拥塞避免**：`cwnd` 线性增长，避免过快导致拥塞。
>    - **拥塞发生**：拥塞时，`cwnd` 减小到 1，阈值减半。
>    - **快速恢复**：重复 ACK 时，`cwnd` 减半，避免完全重置。
>
> 拥塞控制通过动态调整发送速率，平衡网络负载与传输效率。
>
> 
>
> ### 慢启动总结
>
> 1. **目的**：在连接建立后，逐步增加发送数据量，避免一开始发送大量数据导致网络拥塞。
>
> 2. **规则**：每收到一个 ACK，拥塞窗口 `cwnd` 增加 1。
>
> 3. **增长过程**：
>    - 初始 `cwnd = 1`（1 个 MSS）。
>    - 每轮 ACK 到达后，`cwnd` 指数增长（1 → 2 → 4 → 8...）。
>
> 4. **停止条件**：
>    - 当 `cwnd` < `ssthresh`（慢启动门限）时，继续慢启动。
>    - 当 `cwnd` >= `ssthresh` 时，切换到**拥塞避免算法**。
>
> 5. **特点**：慢启动的发送数据量呈**指数增长**，直到达到门限或发生拥塞。
>
> 
>
> ### 拥塞避免算法总结
>
> 1. **进入条件**：当拥塞窗口 `cwnd` 超过慢启动门限 `ssthresh` 时，进入拥塞避免阶段。
>
> 2. **规则**：  
>    - 每收到一个 ACK，`cwnd` 增加 `1/cwnd`。当一轮 ACK 全部到达后，`cwnd` 的**总增量**约为 1。
>    
> 3. **增长特点**：  
>    
>    - 拥塞窗口从指数增长（慢启动）变为**线性增长**，增长速度变慢。
>    
> 4. **示例**：  
>    - 假设 `ssthresh = 8`，当 `cwnd = 8` 时进入拥塞避免。  
>    - 每 8 个 ACK 到达后，`cwnd` 增加 1，下一轮发送 9 个 MSS，依此类推。
>
> 5. **停止条件**：  
>    - 当网络出现拥塞（如丢包或超时重传），进入**拥塞发生算法**。
>
> 6. **特点**：拥塞避免阶段通过缓慢增加发送速率，尽量避免网络拥塞，同时保持数据传输效率。
>
>    
>
> ### 拥塞发生与快速恢复总结
...
>      - 进入快速恢复，反应温和，避免完全回到慢启动。
>
> 2. **快速恢复**：
>    - **进入条件**：触发快速重传后。
>    - **过程**：
>      - `cwnd = ssthresh + 3`，重传丢失数据包。
>      - 每收到重复 ACK，`cwnd += 1`。
>      - 收到新 ACK 后，`cwnd = ssthresh`，进入拥塞避免。
>    - **特点**：恢复速度快，减少网络卡顿。
>
> 3. **对比**：
>    - 超时重传反应激烈，回到慢启动；快速重传+快速恢复反应温和，保持较高流量。
>    
>    - #### 快速回复 收到新的ACK后,就回到 cwnd=新的ssthresh , 开始拥塞避免状态
