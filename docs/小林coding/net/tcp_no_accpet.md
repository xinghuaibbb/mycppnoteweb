...
> ## hzh-c
>
> ### 总结：没有 `accept`，能建立 TCP 连接吗？
>
> 1. **没有 `accept`，TCP 连接是否能建立？**  
>    - **可以**，三次握手的过程与 `accept` 无关。  
>    - 服务端在执行 `listen` 后，内核会创建半连接队列和全连接队列，三次握手完成后，连接会进入全连接队列。  
>    - `accept` 的作用是从全连接队列中取出已建立的连接，供应用程序使用。
>
> 2. **三次握手的细节**  
>    - **第一次握手**：客户端发送 `SYN`，服务端将连接信息放入半连接队列。  
>    - **第二次握手**：服务端回复 `SYN+ACK`，连接仍在半连接队列中。  
>    - **第三次握手**：客户端发送 `ACK`，服务端将连接从半连接队列移至全连接队列。
>
> 3. **半连接队列与全连接队列**  
>    - **半连接队列**：存储未完成三次握手的连接，设计为哈希表以提高查找效率。  
>    - **全连接队列**：存储已完成三次握手的连接，设计为链表以便快速取出连接。
>
> 4. **全连接队列满了会怎样？**  
>    - 默认丢弃第三次握手的 `ACK`，并可能重传第二次握手的 `SYN+ACK`。  
>    - 如果 `tcp_abort_on_overflow=1`，直接返回 `RST`，断开连接。
>
> 5. **半连接队列满了会怎样？**  
>    - 默认丢弃第一次握手的 `SYN`，可能导致连接失败。  
>    - 可通过启用 `tcp_syncookies` 绕过半连接队列，防止 `SYN Flood` 攻击。
>
> 6. **没有 `listen`，能建立连接吗？**  
>    - **可以**，在特殊情况下（如 TCP 自连接或 TCP 同时打开），无需 `listen` 也能建立连接。  
>    - 客户端通过内核的全局哈希表存储连接信息，完成三次握手。
>
> 7. **结论**  
>    - `accept` 仅用于从全连接队列中取出连接，与三次握手无直接关系。  
>    - 没有 `accept`，三次握手仍可正常完成，连接会进入全连接队列等待处理。  
>    - 特殊情况下（如自连接），即使没有 `listen`，也能建立 TCP 连接。
