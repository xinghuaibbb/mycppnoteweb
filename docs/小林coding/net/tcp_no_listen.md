...
> ## hzh-c
>
> ### 总结：服务端未调用 `listen` 和无 `listen` 建立连接的情况
>
> #### 1. **服务端未调用 `listen`，客户端发起连接会发生什么？**
> - 服务端只调用了 `bind`，未调用 `listen`。
> - 客户端发送 `SYN` 报文后，服务端会直接返回 `RST` 报文，表示拒绝连接。
> - 原因：
>   - 服务端未调用 `listen`，没有监听套接字，无法处理连接请求。
>   - 内核在查找监听套接字时找不到对应的 socket，因此返回 `RST` 报文中止连接。
>
> #### 2. **不使用 `listen`，能建立 TCP 连接吗？**
> - **可以**，但有特殊条件：
>   - **TCP 自连接**：客户端自己连接自己（`connect` 自己绑定的地址和端口）。
>   - **TCP 同时打开**：两个客户端同时向对方发起连接请求。
> - 原因：
>   - 连接信息会存储在内核的全局哈希表中，而不是依赖服务端的监听队列。
>   - 在自连接或同时打开的情况下，连接信息可以通过全局哈希表完成匹配，从而建立连接。
>
> #### 3. **实验验证**
> - 服务端未调用 `listen`：
>   - 客户端发起连接时，服务端返回 `RST`，连接失败。
> - TCP 自连接：
>   - 客户端通过 `connect` 自己绑定的地址和端口，成功建立连接。
>   - 数据可以正常发送和接收。
>
> #### 4. **关键点**
> - `listen` 的作用：
>   - 创建半连接队列和全连接队列，用于存储连接信息。
>   - 没有调用 `listen`，服务端无法处理连接请求。
> - 特殊情况（如自连接、同时打开）不依赖服务端监听队列，而是通过内核全局哈希表完成连接。
>
> #### 5. **结论**
> - 服务端未调用 `listen` 时，无法正常接受客户端的连接请求。
> - 在特定场景下（如自连接或同时打开），即使没有 `listen`，也可以建立 TCP 连接。
...
>
> 1. **服务端未调用 `listen` 的情况**  
>    - 客户端发起连接时，服务端会返回 `RST` 报文，连接失败。  
>    - 原因是服务端未监听端口，内核无法找到对应的监听套接字。
>
> 2. **不使用 `listen` 也能建立 TCP 连接的情况**  
>    - **TCP 自连接**：客户端连接自己绑定的地址和端口。  
>    - **TCP 同时打开**：两个客户端同时向对方发起连接请求。  
>    - 这些情况依赖内核的全局哈希表，而非服务端的监听队列。
>
> 3. **`listen` 的作用**  
>    - 创建半连接队列和全连接队列，用于存储连接信息。  
>    - 没有调用 `listen`，服务端无法正常处理连接请求。
>
> 4. **实验验证**  
>    - 服务端未调用 `listen`：客户端连接失败，服务端返回 `RST`。  
>    - TCP 自连接：客户端可以成功连接自己，并正常通信。
>
> 5. **结论**  
>    - 服务端未调用 `listen` 时，无法接受正常的客户端连接。  
>    - 在特殊场景（如自连接、同时打开）下，无需 `listen` 也能建立 TCP 连接。
